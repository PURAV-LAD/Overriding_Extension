...
def rename_file():
    ...
    # Ask for new extension from user
    over_extension = simpledialog.askstring("Input", "Enter the new extension (without dot):", initialvalue="")
    if over_extension is None:
        messagebox.showwarning("No Input", "No new extension provided. Operation canceled.")
        return

    over_extension = over_extension.lower().strip()

    # Unicode character to insert
    unicode_char = '‮'  # Right To left
    # For left To right use ‭
...
rename_button = tk.Button(root, text="Rename File", command=rename_file, bg="palevioletred1", cursor="hand2")
